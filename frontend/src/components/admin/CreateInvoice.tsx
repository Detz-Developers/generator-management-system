...
interface LineItemRow {
  ...
  qty?: number;
  unit_price?: number;
  amount?: number; // if present, overrides qty * unit_price
}

export interface InvoicePayload {
  id: string; // admin-provided unique id
  company_name: string;
  description?: string;
...
interface CreateInvoiceProps {
  ...
  readOnly?: boolean; // when true, disable editing (e.g., Paid invoices)
}
...
export default function CreateInvoice({
  ...
  // Quick add services for convenience
  ...
  const handleSubmit = async () => {
    ...
    const payload: InvoicePayload = {
      ...
      line_items: items.map((it) => ({
        id: it.id,
        description: it.description,
        qty: hasAmountValue(it.amount) ? undefined : toNumber(it.qty, 0),
        unit_price: hasAmountValue(it.amount) ? undefined : toNumber(it.unit_price, 0),
        amount: hasAmountValue(it.amount) ? Number(it.amount) || 0 : undefined,
      })),
    };
    ...
    try {
      if (mode === "edit") {
        const callable = httpsCallable(functions, "updateInvoice");
        await callable({ id: payload.id, patch: { ...payload } });
        setBanner({ type: "success", msg: "Invoice updated" });
      } else {
        const callable = httpsCallable(functions, "createInvoice");
        await callable(payload);
        setBanner({ type: "success", msg: "Invoice created" });
      }
