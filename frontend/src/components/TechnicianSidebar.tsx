...
export default function TechnicianSidebar({ onNavigate, currentPage, onLogout }: TechnicianSidebarProps) {
  const [isCollapsed, setIsCollapsed] = useState(false);

  const menuItems = [
    { name: 'Dashboard', icon: '📊', page: 'Dashboard' },
    { name: 'Tasks', icon: '🔧', page: 'Tasks' },
    { name: 'Services', icon: '⚙️', page: 'Services' },
    ...
    <div className={`bg-white shadow-lg transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'} h-screen flex flex-col`}>
      ...
      <div className="p-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          {!isCollapsed && (
            <div>
              <h1 className="text-lg font-bold text-orange-600">Technician Panel</h1>
