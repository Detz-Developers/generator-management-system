...
export default function UserManagementPage() {
  ...
  const handleSaveUser = async ({ name, email, role: newRole, disabled }: { name: string; email: string; role: RawRole; disabled: boolean }) => {
    ...
    if (editingUser.name !== name || editingUser.email !== email) {
      const updateProfile = httpsCallable(functions, 'updateUserProfile');
      tasks.push(updateProfile({ uid: editingUser.id, name, email }));
    }

    // Update role if changed
    if (editingUser.rawRole !== newRole) {
      const setUserRole = httpsCallable(functions, 'setUserRole');
      tasks.push(setUserRole({ uid: editingUser.id, role: newRole }));
    }
    ...
    if (editingUser.rawStatus !== desiredStatus) {
      ...
      tasks.push(setUserStatus({ uid: editingUser.id, disabled }));
    }
