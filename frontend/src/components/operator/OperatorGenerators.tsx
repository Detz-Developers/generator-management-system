...
import { db } from "@/firebaseConfig";
import { generateAnalysis, generateFallbackReport } from "@/lib/gemini";
...
interface Generator {
  ...
  serial_no: string;
  shop_id: string;
  location: string; // e.g., "DOWN" | "STANDBY" | others => Online
}
...
export default function OperatorGenerators({
  ...
  // Fetch Shop, Generators, Batteries
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
  ...
  const filteredGenerators = generators.filter((g) => {
    ...
    const matchBattery = withBattery ? hasBattery(g.id) : !hasBattery(g.id);
    return matchSearch && matchBattery;
  });
      ...
      {loading ? (
        ...
        <div className="space-y-4 mb-10">
          {filteredGenerators.map((g) => {
            ...
            return (
              <div key={g.id} className={`flex items-center justify-between rounded-lg h-15 shadow ${color}`}>
                <div className="flex items-center gap-4 my-4 mx-2">
                  <span className={`w-6 h-6 rounded-full ${dot}`}></span>
                  <div>
                    <div className="font-bold text-lg">Generator {g.id}</div>
