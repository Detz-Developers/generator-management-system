...
export default function InvoiceManagement({ onNavigate }: Props) {
  ...
  const doDelete = async (id:string) => {
    ...
    setRowBusy(id);
    try {
      const call = httpsCallable(functions, "deleteInvoice");
      await call({ id });
  ...
  const filtered = useMemo(() => { const q=search.trim().toLowerCase(); return invoices.filter(r => (!q || r.id.toLowerCase().includes(q) || r.company_name.toLowerCase().includes(q)) && (status==="All" || r.status===status)); }, [invoices, search, status]);
  const sums = useMemo(() => { const total=invoices.reduce((s,r)=>s+(r.amount||0),0); const paid=invoices.filter(r=>r.status==="Paid").reduce((s,r)=>s+(r.amount||0),0); const pending=invoices.filter(r=>r.status==="Pending").reduce((s,r)=>s+(r.amount||0),0); const overdue=invoices.filter(r=>r.status==="Overdue").reduce((s,r)=>s+(r.amount||0),0); return { total, paid, pending, overdue }; }, [invoices]);

  if (isAdmin===false) return (<div className="flex-1 p-8"><div className="max-w-3xl mx-auto"><div className="bg-red-50 border border-red-200 text-red-800 rounded-lg p-4">Admin only. Please sign in with an admin account.</div></div></div>);

  return (
    <div className="flex-1 p-8">
      {toast && (<div className={`fixed top-4 right-4 z-50 px-4 py-2 rounded-lg shadow-sm border ${toast.type==="success"?"bg-green-50 border-green-200 text-green-800":"bg-red-50 border-red-200 text-red-800"}`}>{toast.msg}</div>)}
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          ...
          <button onClick={openCreate} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2">+ Add Invoice</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-blue-200 p-6 mb-8">
        <div className="flex items-center mb-4">
          <svg className="w-4 h-4 mr-2 text-blue-600" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M3 3a1 1 0 011-1h12a1 1 0 011 1v3a1 1 0 01-.293.707L12 11.414V15a1 1 0 01-.293.707l-2 2A1 1 0 018 17v-5.586L3.293 6.707A1 1 0 013 6V3z" clipRule="evenodd"/></svg>
      ...
      {open && (
        <div className="fixed inset-0 z-40">
          ...
          <div className="absolute inset-y-0 right-0 left-0 md:left-64 bg-white shadow-xl border-l border-blue-200 overflow-auto">
            <CreateInvoice mode={mode} initialId={editData?.id} initialData={editData??undefined} readOnly={readOnly} onClose={()=>setOpen(false)} onSuccess={()=>fetchList()} />
          </div>
        </div>
      )}
    </div>
...
function Card({dot,label,value}:{dot:string;label:string;value:string}){
  return (<div className="bg-white rounded-lg shadow-sm border border-blue-200 p-6"><div className="flex items-center"><div className={`w-3 h-3 ${dot} rounded-full mr-3`}></div><div><p className="text-sm text-gray-500">{label}</p><p className="text-xl font-bold">{value}</p></div></div></div>);
}
